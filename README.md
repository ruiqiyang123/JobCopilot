...
### 4. 使用
**开始收集 + AI 筛选** → 查看完整岗位并逐个或 **一键批准推荐岗位** → **预演已批准岗位** → 编辑、重新生成并确认 AI 开场 → **正式投递已批准岗位**。正式投递会安全关闭已识别的订阅回复弹窗，等待并核对实际聊天页；聊天页持续缺少岗位 ID 时，会在发送前自动重新打开并完整复核当前岗位一次，随后再按顺序发送三段内容。文字或图片发送失败时不会自动重试。
