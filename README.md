...
- 🔗 **聊天页交接**：支持同标签和新标签跳转，核对岗位 ID 后才发送，不再把正常页面跳转误报为消息通道关闭
...
### 4. 使用
**开始收集 + AI 筛选** → 查看完整岗位并 **批准投递** → **预演已批准岗位** → 编辑、重新生成并确认 AI 开场 → **正式投递已批准岗位**。正式投递会等待并核对实际聊天页，再按顺序发送三段内容。
...
- 审核状态、预演内容和岗位进度保存在浏览器扩展本地存储中；不会扫描 BOSS 历史会话。
