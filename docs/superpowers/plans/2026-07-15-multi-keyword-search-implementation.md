...
- [x] 先增加失败集成断言，验证搜索模式、扩展开关、唯一岗位上限文案及脚本加载顺序。
- [x] 增加精准/平衡/宽松模式和自动扩展开关；旧 `keyword`、`count` 自动沿用，默认平衡模式并开启扩展。
- [x] 将“收集岗位数量”改名为“唯一岗位收集上限”，关键词输入支持逗号、顿号、分号和换行。
- [x] 版本升至 `1.3.0`，同步 README。
- [x] 运行全部测试、语法、Manifest、差异和密钥扫描。
- [x] 更新勾选、提交并推送现有 PR；不访问 BOSS，不执行真实投递。
