...
- 🔗 **聊天页交接**：兼容订阅回复弹窗和短暂无岗位 ID 的聊天壳，只向 `jobId` 匹配的聊天页发送
...
正式投递会再次读取岗位并校验身份、筛选资格和稳定预演配置；遇到已识别的“订阅回复消息”弹窗时会关闭并只重试一次。聊天页短暂缺少 `jobId` 时最多等待 5 秒，只有精确匹配岗位后才按“AI 开场 → 固定补充消息 → 简历图片”逐段发送。任一真实失败仍会立即停止，剩余岗位不会自动继续或重试。
