...
- 🔗 **聊天页交接**：点击沟通前启动页面观察，支持同标签和新标签跳转，只向 `jobId` 匹配的聊天页发送
...
正式投递会再次读取岗位并校验身份、筛选资格和稳定预演配置，然后等待并核对实际聊天页的 `jobId`，最后按“AI 开场 → 固定补充消息 → 简历图片”逐段发送。同标签、新标签和导航造成的瞬时消息通道关闭都可正确处理；任一真实失败仍会立即停止，剩余岗位不会自动继续或重试。
...
- 岗位进度和状态历史保存在浏览器扩展本地存储中，不会主动扫描 BOSS 历史会话。
