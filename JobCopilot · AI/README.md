...
- 🧪 **完整预演与冻结**：展示最终三段内容，AI 开场可编辑；确认后正式投递不再临时重新生成
...
4. 点击 **预演已批准岗位**，检查并按需编辑每个岗位的 AI 开场，然后点击 **确认此岗位预演**。
5. 点击 **正式投递 N 个已批准岗位** 并确认批次名单。

正式投递会再次读取岗位并校验身份、筛选资格和预演有效性，然后按“AI 开场 → 固定补充消息 → 简历图片”逐段发送。任一失败会立即停止，剩余岗位不会自动继续或重试。
